...
import com.hsp302.shared_english_e_learning_path.domain.enums.*;
...
import com.hsp302.shared_english_e_learning_path.repositories.*;
    ...
    public void importBlogsFromExcel(InputStream inputStream) throws IOException {
        ...
        for (int i = 1; i <= sheet.getLastRowNum(); i++) {
            ...
            try {
                ...
                BlogType type = BlogType.valueOf(getCellValue(row.getCell(3)).toUpperCase());
                BlogStatus status = BlogStatus.valueOf(getCellValue(row.getCell(4)).toUpperCase());
                String memberUsername = getCellValue(row.getCell(5));
                User member = userService.getUserEntity(memberUsername);
                String content = getCellValue(row.getCell(6));
                Integer time = Integer.valueOf(getCellValue(row.getCell(7)));
                AgeGroup group = AgeGroup.valueOf(getCellValue(row.getCell(8)));
